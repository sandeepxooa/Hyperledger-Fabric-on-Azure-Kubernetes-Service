...
async function provisionFabricToolsContainer(options, logger) {
  ...
  logger.info(`provisionFabricToolsContainer called with options `, options);
  ...
  logger.info(
    `Successfully loaded KubernetesClusterAdminCredential from env variables`
  );
  ...
  await createIfNotExist(
    client.api.v1.namespaces(toolsNamespace).secrets,
    manifestFtSecretCaCredentials,
    logger
  );
  ...
  if (process.env.hlfCaRootCertPrivateKey) {
    ...
    manifestFtSecretCaPrivateCertificate.stringData["rca.key"] = decode(
      process.env.hlfCaRootCertPrivateKey
    );
    await createIfNotExist(
      client.api.v1.namespaces(toolsNamespace).secrets,
      manifestFtSecretCaPrivateCertificate,
      logger
    );
  }
  ...
  if (process.env.hlfCaRootCertPublicKey) {
    ...
    manifestFtSecretCaPublicCertificate.stringData["rca.pem"] = decode(
      process.env.hlfCaRootCertPublicKey
    );
    await createIfNotExist(
      client.api.v1.namespaces(toolsNamespace).secrets,
      manifestFtSecretCaPublicCertificate,
      logger
    );
  }
  ...
  const fs = require("fs");
  const cfg = yaml.safeLoad(
    fs.readFileSync(
      "./aksManifests/fabricTools/container-azm-ms-agentconfig.yaml",
      "utf8"
    )
  );
  await createIfNotExist(
    client.api.v1.namespaces("kube-system").configmap,
    cfg,
    logger
  );
  ...
  await createIfNotExist(
    client.api.v1.namespaces(toolsNamespace).serviceaccounts,
    manifestFtServiceAccount,
    logger
  );
  ...
  await createIfNotExist(
    client.api.v1.namespaces(toolsNamespace).pods,
    manifestFtPod,
    logger
  );
}
